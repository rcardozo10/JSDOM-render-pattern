...
function render() {
  ...
  if (content === "") {
    cartItem.innerText = "Nothing in cart";
  } else {
    cartItem.innerHTML = content;
  }
  var totalRender = document.querySelector("#cartTotal > .total-price");
  totalRender.innerHTML = "";
  var getContent = calculateTotal();
  if (getContent === 0) {
    totalRender.innerText = "$0";
  } else {
    totalRender.innerText = "$".concat(getContent);
  }
}
...
*/
var sortCart = function sortCart(priceA, priceB) {
  var priceACost = parseFloat(priceA.price);
  var priceBCost = parseFloat(priceB.price);
